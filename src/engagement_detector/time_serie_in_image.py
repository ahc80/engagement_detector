...
class TimeSerieInImage(object):
    ...
    def __init__(self):
        ...
        try:
            self.font_small = ImageFont.truetype("arial", 10)
        except OSError:
            self.font_small = ImageFont.load_default()

        try:
            self.font_large = ImageFont.truetype("arial", 16)
        except OSError:
    ...
    def step(self, frame, value):
        ...
        grid_draw.text((0, plot_offset - 11), "1.0", font=self.font_small, fill=(255, 255, 255))
        grid_draw.text((0, plot_offset + plot_max_height - 11), "0.0", font=self.font_small, fill=(255, 255, 255))
        ...
        xy_pre = list(zip(x_pre, y_pre))
        xy_post = list(zip(x_post, y_post))
        ...
        plot_draw.text((width / 2., plot_offset - 20), "%.2f" % value, font=self.font_large, fill=self.pre_color)
